...
columnNames = ['song_id', 'segment_number', 'segment_start', 'segment_confidence',
               'loudness_max', 'loudness_max_time', 'loudness_max_start' ] + 
              ['pitch_basis' + str(i) for i in range(12)] +
              ['timbre_basis' + str(i) for i in range(12)]
columnTypes = ['STRING_TYPE', 'LONG_TYPE'] + ['DOUBLE_TYPE'] * 29
...
destination = myria.MyriaRelation("Jeremy:MSD:SegmentsTable", 
                            schema=schema,
...
filename = 'pitch_beat_aligned_table.tsv'
upload_files = []
for i in range(1,27) :
